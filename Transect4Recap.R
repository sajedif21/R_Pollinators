...
for.net.data  <- read.xlsx("final data.xls", sheetName = "Transect4")
datas <- read.csv("Transect4.csv",header = TRUE)
datas
...
complete.network2 <- complete.network[1:2, 1:2] ; dim (complete.network2)
rownames(datas) <- c("Ageratina pseudochilca","Ilex")
